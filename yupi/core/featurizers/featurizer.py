...
class Featurizer(abc.ABC):
    ...
    def describe(self, detailed: bool = False, start_index: int = 0) -> Tuple[int, List[Tuple[str, int, int]]]:
        """Prints a description of the featurizer, including the number of features and optionally their column indices.

        Parameters:
            detailed (bool): If True, include the start and end indices of the features contributed by this featurizer.
                                    Defaults to False.
            start_index (int): The starting index for the features (used for CompoundFeaturizers). Defaults to 0.

        Returns:
            Tuple[int, List[Tuple[str, int, int]]]: A tuple containing the next available start index and a list of tuples
                                                    with the featurizer name, start index, and end index.
        """
...
class CompoundFeaturizer(Featurizer):
    ...
    *featurizers : Featurizer
    ...
    def describe(self, detailed: bool = False, start_index: int = 0) -> Tuple[int, List[Tuple[str, int, int]]]:
        ...
        """
        if detailed:
            print(f"{self.__class__.__name__}: Contributes features from column {start_index} to {start_index + self.count - 1}")
        else:
            print(f"{self.__class__.__name__}: Contributes {self.count} features.")
        ...
        for featurizer in self.featurizers:
            current_index, ranges = featurizer.describe(detailed, current_index)
            feature_ranges.extend(ranges)
        return current_index, feature_ranges
